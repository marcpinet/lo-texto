/* eslint-disable prettier/prettier */
import {BleManager} from 'react-native-ble-plx';
import {Alert, PermissionsAndroid,LogBox} from 'react-native';
import base64 from 'react-native-base64';
import {chatData} from '../assets/data/chatData';
import {saveChat} from './Situations';
const manager = new BleManager();

var d = null;
//initialize ble
export const initializeBle = () => {
  LogBox.ignoreLogs(['new NativeEventEmitter']);
  LogBox.ignoreLogs(['Possible Unhandled Promise Rejection']);
  var states;
  PermissionsAndroid.request(
    PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
    {
      title: 'Bluetooth Permission',
      message: 'Bluetooth Permission',
      buttonPositive: 'Oui',
      buttonNegative: 'Non',
    },
  ).then(() => {
    //check if bluetooth is available
    manager.state().then(state => {
      if (state !== 'PoweredOn') {
        states = state;
        alerte();
      } else {
        manager.states = state;
        scan();
      }
    });

    manager.onStateChange(state => {
      if (state === 'PoweredOn' && states !== state) {
        states = state;
      } else if (state !== 'PoweredOn' && states !== state) {
        states = state;
        alerte();
      }
    });
  });
};

const alerte = () => {
  manager.stopDeviceScan();
  manager.state().then(state => {
    Alert.alert('Bluetooth is not available', 'Turn on bluetooth', [
      {
        text: 'Ok',
        onPress: () => {
          if (state !== 'PoweredOn') {
            alerte();
          } else {
            scan();
          }
        },
      },
    ]);
  });
};

//check if the device disconnect
const checkDisconnect = (devices) => {
  var dis = manager.onDeviceDisconnected(devices.id,() => {
    scan();
    dis.remove();
  });
};

//Scan for bluetooth deivces
const scan = () => {
  manager.startDeviceScan(null, null, (error, device) => {
    if (error) {
      console.log(error);
    }
    if (device) {
      if (device.name === 'MLT-BT05') {
        manager.stopDeviceScan();
        manager.connectToDevice(device.id).then(deviced => {
          manager
            .discoverAllServicesAndCharacteristicsForDevice(deviced.id)
            .then(() => {
              d = deviced;
              checkDisconnect(d);
              receiveMessage();
            });
        });
      }
    }
  });
};

//Send message to connected device
export const sendMessage = messages => {
  if (d == null) {
    Alert.alert('No device connected', 'Please wait for a device to connect');
  } else {
    manager.writeCharacteristicWithoutResponseForDevice(
      d.id,
      'FFE0',
      'FFE1',
      base64.encode(messages + '|'),
    );
  }
};

//Receive message from connected device
const receiveMessage = () => {
  const regex = /^[A=]*$/;
  manager.monitorCharacteristicForDevice(
    d.id,
    'FFE0',
    'FFE1',
    (error, characteristic) => {
      if (error) {
        console.log(error);
      }
      if (!regex.test(characteristic.value)) {
        chatData.push(base64.decode(characteristic.value));
        saveChat();
      }
    },
  );
};
